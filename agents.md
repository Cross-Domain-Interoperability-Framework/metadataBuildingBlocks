...
profile's own field order. This is the same categorisation the ADA metadata forms use, and it is
generated by `convert_for_jsonforms.py` from the profiles — not maintained here. A record matches a
...
Two things this surfaced. **A uischema outlives its profile**: `XASdata`'s composite was archived in
July 2026 in favour of `xasDocument`, but `_sources/jsonforms/profiles/cdifProfiles/XASdata/` is
still there while `xasDocument` has none — so matching on the directory name alone would apply a
retired profile's layout. Layout discovery therefore requires a live composite under
`_sources/profiles/cdifCompositeProfile/`, which skips `XASdata`. And **the CoreDiscovery uischema
does not place five properties its profile declares** — `schema:sameAs`,
`schema:measurementTechnique`, `dqv:hasQualityMeasurement`, `prov:wasGeneratedBy`,
`prov:wasDerivedFrom` — which land in "Additional" rather than vanishing. Worth fixing in
`convert_for_jsonforms.py`.
