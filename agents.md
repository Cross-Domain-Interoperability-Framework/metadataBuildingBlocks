...
- Two-pass `$defs` resolution: pass 1 resolves external file refs with empty defs dict, pass 2 uses `_inline_unresolved_defs` to replace `$comment` placeholders left by forward cross-def fragment refs. `_inline_unresolved_defs` recurses into the replacement (so nested placeholders in deeply-chained defs get resolved) and merges sibling keys (e.g. a `description` declared next to the `$ref`) onto the inlined body via `deep_merge`.
- **Cycle handling (resolved mode):** when `_inline_unresolved_defs` detects a cycle via the `resolving` set, it emits a typed stub `{"type": "object", "$comment": "cycle: <DefName>", ...siblings}` rather than leaving an opaque placeholder. The stub is schema-valid (matches any object) and clearly labels where the cycle was cut.
- **Cycle handling (structured mode, `--structured`):** local fragment refs to inline `$defs` are always promoted to top-level `$defs` of the structured output (with disambiguated names if there's a collision with BB-level promotions); cross-file fragment refs to inline `$defs` are also promoted instead of falling through to whole-file resolution. `_resolve_promoted_defs` resolves each promoted entry into its $def body, iterating until the promotion set is stable. Cycles are then expressed as `$ref: #/$defs/<name>` — the canonical JSON Schema way to handle recursion. `inline_low_use_defs` collapses non-cyclic, low-use defs back inline; `_is_in_cycle` (graph-reachability check via `_has_ref_to`) excludes cyclic defs to avoid producing dangling self-refs.
