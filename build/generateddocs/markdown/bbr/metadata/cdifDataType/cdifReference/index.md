...
`cdif.bbr.metadata.cdifDataType.cdifReference` *v0.2*
...
```jsonld
{
  "@context": [
    ...
    "https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/build/annotated/bbr/metadata/cdifDataType/cdifReference/context.jsonld",
    {
      "schema": "http://schema.org/",
      "dcterms": "http://purl.org/dc/terms/",
      "dcat": "http://www.w3.org/ns/dcat#",
...
$defs:
  ...
  Relation:
    ...
    description: 'DCAT Relationship surface added on top of the schema.org labeled
      link construct. This block is an allOf of labeledLink and this surface, so a
      "@type" with contains: dcat:Relationship here applied to EVERY cdifReference
      -- alongside labeledLink''s contains: schema:CreativeWork -- making the two
      co-types mandatory and the block unsatisfiable for an ordinary labeled link.
      schema:license recommends exactly such a link ("recommend using schema:CreativeWork
      to provide a label (name) for the license"), and every example that followed
      that advice failed. The co-type mandate is therefore gone: @type is governed
      solely by labeledLink, which already requires schema:CreativeWork and permits
      dcat:Relationship alongside it. This matches rules.shacl, where cdifReferenceShape
      targets dcat:Relationship as a class -- the relation rules apply to nodes that
      declare it, rather than forcing every labeled link to.'
    properties:
      dcat:hadRole:
...
* YAML version: [schema.yaml](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/build/annotated/bbr/metadata/cdifDataType/cdifReference/schema.json)
* JSON version: [schema.json](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/build/annotated/bbr/metadata/cdifDataType/cdifReference/schema.yaml)
...
[context.jsonld](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/build/annotated/bbr/metadata/cdifDataType/cdifReference/context.jsonld)
...
* Path: `_sources/cdifDataType/cdifReference`
