...
| `CDIFDataDescriptionProfile` | CDIF Data Description profile (allOf: cdifCore + cdifDataDescription + discovery properties). Flat: InstanceVariables with `cdif:role`, `cdi:qualifies`, value-domain links (`cdi:takesSentinelValuesFrom` → `cdif:SentinelValueDomain`, `cdi:takesSubstantiveValuesFrom` → `cdif:SubstantiveValueDomain`), and `cdif:isDescribedBy_StatisticsCollection`. Dataset-level extensions: `cdif:hasPrimaryKey` (ordered `cdi:ComponentPosition` wrappers around InstanceVariables), `cdif:statistics`. WebAPI distributions: `schema:potentialAction.schema:result` can be typed `cdi:PhysicalDataSet` and carry the same physical-realization properties (`cdif:hasPhysicalMapping`, `cdi:characterSet`, `cdif:fileSize`/`fileSizeUofM`) as a sibling DataDownload distribution. No component classes. |
