...
This is a CDIF profile of the DDI-CDI `EnumerationDomain` extension point. It is intentionally light at the root level (only `@type` is required) so that any codification — whether expressed as a SKOS `ConceptScheme`, a DDI-CDI `CodeList`, `StatisticalClassification`, or an inline `CategorySet` — can be advertised as a domain with the same shape. The local `$defs` block carries the supporting DDI-CDI types (`Category`, `CategoryPosition`, `CategorySet`, `ClassificationItem`, `Level`, `LevelStructure`, `Notation`, `AuthorizationSource`, and `TypedString`) for cases where the domain content is described inline.
...
- `schema:inDefinedTermSet` — anyOf a `skos:ConceptScheme` reference (inline or `@id`) or an `@id`-only id-reference to a domain defined elsewhere.
