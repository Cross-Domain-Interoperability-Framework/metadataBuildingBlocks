...
This is a CDIF profile of DDI-CDI's `Key` / `PrimaryKey` concept. Where the canonical DDI-CDI form composes a Key from `cdi:DataStructureComponent` nodes, the CDIF profile composes it from `cdi:InstanceVariable` nodes (referenced through the `cdifVariableMeasured` building block) so the Key references the actual measured variables rather than an intermediate component layer.
...
- Each `cdif:ComponentPosition` carries:
  - `cdif:indexes` — the `cdi:InstanceVariable` (inline `cdifVariableMeasured` node, or an `@id`-only reference);
