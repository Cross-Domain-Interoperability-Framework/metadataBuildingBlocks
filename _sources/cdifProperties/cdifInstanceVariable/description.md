...
- **cdif:role** — role of variable in data structure (`UnitIdentifier`, `Measure`, `Attribute`, `Dimension`, `Descriptor`, `ReferenceVariable`)
...
- **cdi:qualifies** — `@id` reference to another instance variable in the same dataset; required when `cdif:role` is `Attribute`
